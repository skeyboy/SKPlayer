...
private class ProgressHUD: NSView {
    ...
    override func draw(_ rect: NSRect) {
        ...
        case .determinate:
            ...
            processBackgroundPath.lineCapStyle = NSBezierPath.LineCapStyle.roundLineCapStyle
            ...
            processPath.lineCapStyle = NSBezierPath.defaultLineCapStyle
...
private class ProgressIndicatorLayer: CALayer {
    ...
    private func setupAnimTimer() {
        ...
        if let aTimer = animationTimer {
            RunLoop.current.add(aTimer, forMode: RunLoopMode.commonModes)
        }
        if let aTimer = animationTimer {
            RunLoop.current.add(aTimer, forMode: RunLoopMode.defaultRunLoopMode)
        }
        if let aTimer = animationTimer {
            RunLoop.current.add(aTimer, forMode: RunLoopMode.eventTrackingRunLoopMode)
        }
    }
